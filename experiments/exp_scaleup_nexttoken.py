"""
Scale-up 141M + Next-token Prediction Pre-training + 5 Task Eval

Next-token: 시계열을 여러 구간으로 나눠서 "다음 구간 예측"
  [1..32] → predict [33..64]
  [1..64] → predict [65..96]
  = autoregressive하게 forecasting을 직접 학습

이게 masked recon보다 forecasting에 유리한 이유:
  masked recon: 입력 "안"의 빈 곳 복원 → extrapolation 못 배움
  next-token: 입력 "밖"의 미래 예측 → forecasting 직접 학습

사용법:
  CUDA_VISIBLE_DEVICES=2 python experiments/exp_scaleup_nexttoken.py 2>&1 | tee log/scaleup_nexttoken.log
"""
import sys, os
...
def get_141m_args():
    ...
    return SimpleNamespace(
        ...
        deeponet_width=192,       # 128 → 192
        n_experts=4,
        branch_depth=4,
        trunk_depth=2,
        activation='gelu',
        dropout=0.1,
        branch_hidden=768,        # 512 → 768
...
def pretrain_nexttoken(model, device, save_path, epochs=20, lr=0.0003):
    """Next-token prediction pre-training.

    For each sample [96 timesteps]:
      - Random split point p in [32, 64]
      - Input: first p timesteps (zero-padded to 96)
      - Target: next (96-p) timesteps
      - Also do masked recon on full sequence
    """
    print(f'\n{"="*60}')
    print('Pre-training: Next-token Prediction + Masked Recon')
    print(f'{"="*60}')
    ...
    for epoch in range(epochs):
        ...
        for i, batch_x in enumerate(train_dl):
            batch_x = batch_x.float().to(device)  # [B, 96, 1]
            B, S, C = batch_x.shape
            optimizer.zero_grad()

            # 1) Next-token prediction
            # Random split point
            split = torch.randint(24, 72, (1,)).item()  # context: 24~72, predict rest
            context = batch_x[:, :split, :]  # [B, split, 1]
            target = batch_x[:, split:, :]   # [B, 96-split, 1]
            target_len = S - split

            # Zero-pad context to seq_len
            context_padded = F.pad(context, (0, 0, 0, S - split))  # [B, 96, 1]

            # Predict future
            nt_out = model(context_padded, None, None, None, target_pred_len=target_len)
            if isinstance(nt_out, tuple):
                nt_out = nt_out[0]
            nt_loss = F.mse_loss(nt_out, target)

            # 2) Masked reconstruction (보조)
            mask_rate = 0.4
            ...
            # Combined: next-token이 메인, recon이 보조
            loss = nt_loss + 0.5 * recon_loss
            ...
            if (i + 1) % 500 == 0:
                print(f'  iter {i+1}/{len(train_dl)}: nt={np.mean(nt_losses[-500:]):.4f} recon={np.mean(recon_losses[-500:]):.4f}')

        scheduler.step()
        train_loss = np.mean(losses)
        lr_now = scheduler.get_last_lr()[0]
        print(f'Epoch {epoch+1}/{epochs}: loss={train_loss:.4f} (nt={np.mean(nt_losses):.4f} recon={np.mean(recon_losses):.4f}) lr={lr_now:.6f} ({time.time()-t0:.0f}s)')
...
if __name__ == '__main__':
    ...
    print(f'Model: {n_params/1e6:.1f}M params (width={args.deeponet_width}, hidden={args.branch_hidden})')

    save_path = 'checkpoints/scaleup_141m_nexttoken.pth'
    os.makedirs('checkpoints', exist_ok=True)

    model = pretrain_nexttoken(model, device, save_path, epochs=20, lr=0.0003)
    fc = eval_forecasting(model, device)
    cls = eval_classification(model, device)
    imp = eval_imputation(model, device)

    print(f'\n{"="*60}')
    print(f'FINAL: 141M + Next-token + Pile')
    print(f'{"="*60}')
    print('\nForecasting:')
