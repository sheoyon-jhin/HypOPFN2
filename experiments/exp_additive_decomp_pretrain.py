"""
Additive Decomposition Model: Real+Synth Pre-training + 5 Task Eval

아키텍처:
  Shared Encoder → z → 4개 diverse head → 4개 diverse trunk → additive sum
    - Fourier trunk: 주기적 패턴
    - RBF trunk: 국소적 변동
    - Polynomial trunk: 추세
    - Wavelet trunk: 멀티스케일

Config: medium (6.5M), Pile+Synth, 20 epoch
  - Masked recon + Next-token prediction

사용법:
  CUDA_VISIBLE_DEVICES=1 python experiments/exp_additive_decomp_pretrain.py 2>&1 | tee log/additive_decomp_pretrain.log
"""
import sys, os
...
class TrunkOperator(nn.Module):
    ...
    def __init__(self, width, trunk_depth, trunk_type, n_freq=32, n_rbf=16):
        ...
        if trunk_type == 'fourier':
            self.trunk_input_dim = 1 + 2 * n_freq
        elif trunk_type == 'rbf':
    ...
    def forward(self, head_output, target_len):
        ...
        Phi = self.get_trunk_features(t).unsqueeze(0).expand(B, -1, -1)
...
class AdditiveDecompModel(nn.Module):
    ...
    def _forward_channel(self, x_ch, x_cross, target_len, mode='forecast'):
        ...
        for head, trunk_op in zip(heads, self.trunk_ops):
            component = trunk_op(head(z), target_len)
...
def pretrain(model, device, save_path, epochs=20, lr=0.0003, mask_rate=0.4):
    print(f'\n{"="*60}')
    print(f'Pre-training: Additive Decomp (Pile + Synth)')
    print(f'{"="*60}')
...
if __name__ == '__main__':
    print('=' * 60)
    print('Additive Decomposition Model: Pre-train + 5 Task Eval')
    print('  Architecture: Shared Encoder + 4 Diverse Trunk Operators')
    print('  Config: medium (width=96, hidden=384)')
    print('  Data: Pile (Real) + TempoPFN (Synthetic)')
    print('  Pre-train: Masked Recon + Next-token, 20 epochs')
    print('=' * 60)
    ...
    save_path = 'checkpoints/additive_decomp_medium.pth'
