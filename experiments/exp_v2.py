"""
V2 Training: SEQ_LEN=192 + Last Value Residual + Scaled Up

Two variants controllable by --decomp flag:
  --decomp=False: single shared encoder + 3 trunks (Fourier/Poly/RBF)  [32.5M-style scaled up]
  --decomp=True:  4-way input decomp + 4 per-component encoders + matched trunks [L7-style]

Key improvements over v1:
  ✓ SEQ_LEN: 96 → 192 (2x context, helps high-freq data like ETTm1)
  ✓ Last value residual (NLinear-style): model predicts delta from last observation
  ✓ Scaled up: trunk_width 192 → 256, n_layers 6 → 8 (for no-decomp)
  ✓ Target: ~55M params

Usage:
  CUDA_VISIBLE_DEVICES=0 python experiments/exp_v2.py --decomp 0 --tag nodecomp
  CUDA_VISIBLE_DEVICES=2 python experiments/exp_v2.py --decomp 1 --tag decomp
"""
...
SEQ_LEN = 192         # ← 96 → 192
PATCH_SIZE = 16
HIDDEN = 512          # main encoder d_model
N_LAYERS = 8          # 6 → 8
WIDTH = 256           # trunk width 192 → 256
HIDDEN_DECOMP = 384   # per-component encoder d_model (decomp only)
N_LAYERS_DECOMP = 4   # per-component layers (decomp only)
N_FREQ = 32
N_RBF = 30
RBF_BW = 300          # narrower RBF for longer context
MA_WINDOW = 25
FFT_TOP_K_SEASON = 3
CYCLE_LOW = 4
CYCLE_HIGH = 30
...
# ============================================================
# Differentiable decomposition (for decomp=True)
# ============================================================
...
# Trunks
# ============================================================
TOP_K_IQ = 5   # top-k frequencies for informed query
INFORMED_DIM = 1 + 2*TOP_K_IQ + 2  # t + sin/cos(top-k) + last_val + last_slope = 13


def extract_freq(x, top_k=TOP_K_IQ):
    """x: [B, L] → (freqs [B, k], phases [B, k], last_val [B, 1], slope [B, 1])"""
    fft = torch.fft.rfft(x, dim=-1)
    mag = fft.abs(); mag[:, 0] = 0
...
def build_iq(t_col, freqs, phases, lv, ls):
    """Build informed query features. t_col: [nq, 1]; freqs/phases: [B, k]; lv/ls: [B, 1].
    Returns: [B, nq, informed_dim] where informed_dim = 1+2k+2 = 13."""
    B = freqs.shape[0]
    T = t_col.shape[0]
    t_exp = t_col.squeeze(-1).unsqueeze(0).unsqueeze(-1)   # [1, T, 1]
    f = freqs.unsqueeze(1)                                  # [B, 1, k]
    p = phases.unsqueeze(1)                                 # [B, 1, k]
    ang = 2 * math.pi * f * t_exp + p                       # [B, T, k]
    return torch.cat([
        t_col.squeeze(-1).unsqueeze(0).expand(B, -1).unsqueeze(-1),   # [B, T, 1]
        torch.sin(ang), torch.cos(ang),                                # [B, T, 2k]
        lv.unsqueeze(1).expand(-1, T, -1),                             # [B, T, 1]
        ls.unsqueeze(1).expand(-1, T, -1),                             # [B, T, 1]
    ], dim=-1)
...
class HTrunk(nn.Module):
    def __init__(self, w, btype, nf=N_FREQ, deg=6, nc=N_RBF, bw=RBF_BW):
        ...
        # Full input dim for trunk: base basis + informed query
...
def hfwd_iq(trunk, t_flat, head_out, iq_features):
    """trunk: HTrunk, t_flat: [B*nq], head_out: [B, odim], iq_features: [B, nq, INFORMED_DIM]"""
    B = head_out.shape[0]
    base = trunk.base_feat(t_flat)            # [B*nq, idim]
    nq = base.shape[0] // B
    base = base.view(B, nq, trunk.idim)
    full = torch.cat([base, iq_features], dim=-1)  # [B, nq, full_idim]
...
class TrunkBlock(nn.Module):
    ...
    def forward(self, x_part, qt, iq_features):
        """x_part: [B, L] input to encoder, iq_features: [B, nq, INFORMED_DIM] shared."""
...
def build_iq_for_model(ctx_n, qt):
    """Extract informed query from normalized input and build iq features.
    ctx_n: [B, L], qt: [B, nq] → iq_features: [B, nq, INFORMED_DIM]"""
    freqs, phases, lv, ls = extract_freq(ctx_n)
    B, nq = qt.shape
    # Per-sample t values — build iq per-sample
    t_exp = qt.unsqueeze(-1)                      # [B, nq, 1]
    f = freqs.unsqueeze(1)                         # [B, 1, k]
    p = phases.unsqueeze(1)                        # [B, 1, k]
    ang = 2 * math.pi * f * t_exp + p              # [B, nq, k]
    iq = torch.cat([
        t_exp,                                                # [B, nq, 1]
        torch.sin(ang), torch.cos(ang),                       # [B, nq, 2k]
        lv.unsqueeze(1).expand(-1, nq, -1),                   # [B, nq, 1]
        ls.unsqueeze(1).expand(-1, nq, -1),                   # [B, nq, 1]
    ], dim=-1)
    return iq


class ModelV2NoDecomp(nn.Module):
    """Single encoder + 3 trunks + informed query + last value residual."""
    def __init__(self):
        super().__init__()
    ...
    def _forward_normalized(self, ctx_n, qt):
        iq = build_iq_for_model(ctx_n, qt)     # informed query from normalized input
        return self.block(ctx_n, qt, iq)

    def forward_train(self, ctx, qt):
        """ctx: [B, 192] pre-normalized. Last value residual."""
        last = ctx[:, -1:]                        # [B, 1]
        ctx_c = ctx - last                        # center on last value
...
class ModelV2Decomp(nn.Module):
    """4-way decomp + 4 per-component encoders + matched trunks + informed query + last value residual."""
    def __init__(self):
        super().__init__()
        self.block_T = TrunkBlock(PatchAttnEncoder(HIDDEN_DECOMP, N_LAYERS_DECOMP), ['poly'], HIDDEN_DECOMP)
        self.block_S = TrunkBlock(PatchAttnEncoder(HIDDEN_DECOMP, N_LAYERS_DECOMP), ['fourier'], HIDDEN_DECOMP)
        self.block_C = TrunkBlock(PatchAttnEncoder(HIDDEN_DECOMP, N_LAYERS_DECOMP), ['chirplet'], HIDDEN_DECOMP)
        self.block_R = TrunkBlock(PatchAttnEncoder(HIDDEN_DECOMP, N_LAYERS_DECOMP), ['rbf'], HIDDEN_DECOMP)

    def _forward_normalized(self, ctx_n, qt):
        # Informed query from the ORIGINAL (un-decomposed) normalized input
        iq = build_iq_for_model(ctx_n, qt)
        trend, season, cycle, resid = decompose_4way(ctx_n)
        return (self.block_T(trend, qt, iq) + self.block_S(season, qt, iq) +
                self.block_C(cycle, qt, iq) + self.block_R(resid, qt, iq))
...
# ============================================================
# Datasets (SEQ_LEN 192 means windows of 384 for forecast: 192 ctx + 192 future)
# ============================================================
WINDOW_LEN = SEQ_LEN * 2   # 384


class StreamingPileDataset(Dataset):
    def __init__(self, max_samples=2000000, skip_anomaly=True):
...
class TempoPFNDataset(Dataset):
    def __init__(self, max_samples=400000):
        ...
        for i in range(len(table)):
            ...
            elif len(ts) >= WINDOW_LEN:
                s = np.std(ts[:WINDOW_LEN])
                if s > 1e-6:
                    self.windows.append(np.clip((ts[:WINDOW_LEN]-np.mean(ts[:WINDOW_LEN]))/s, -10, 10).astype(np.float32))
...
# Sampling
# ============================================================
def collate_batch(windows, mode='forecast', n_query=24, mr=0.375):
    ctxs, qts, qvs = [], [], []
    for w in windows:
...
def train(model, datasets, save_path, epochs=20, lr=3e-4, batch_size=64):
    ...
    print(f'\n{"="*60}')
    print(f'V2 Pre-training ({"decomp" if isinstance(model, ModelV2Decomp) else "no-decomp"})')
    print(f'  Params: {n/1e6:.1f}M')
    print(f'  SEQ_LEN: {SEQ_LEN}, WIDTH: {WIDTH}')
    print(f'  Last value residual: ON')
    print(f'{"="*60}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--decomp', type=int, default=0, help='0: no-decomp, 1: decomp')
    parser.add_argument('--tag', type=str, default='v2')
    parser.add_argument('--pile_max', type=int, default=1500000)
    parser.add_argument('--tempopfn_max', type=int, default=300000)
    args = parser.parse_args()

    np.random.seed(42); torch.manual_seed(42)

    print('='*60)
    print(f'V2 Pre-training [decomp={bool(args.decomp)}, tag={args.tag}]')
    print('='*60)
    ...
    print(f'\nTotal: {sum(len(d) for d in datasets):,} windows')

    model = ModelV2Decomp() if args.decomp else ModelV2NoDecomp()
    n = sum(p.numel() for p in model.parameters())
    print(f'\nModel: {n/1e6:.1f}M params')
    model = model.to(DEVICE)

    save_path = f'checkpoints/v2_{args.tag}.pth'
