"""
63M MoE Full Eval: Head fine-tune for ALL tasks
- Forecasting: grid search best config (lr=0.0001, wd=0.001, bs=64)
- Imputation: head fine-tune
- Classification: linear probe

CUDA_VISIBLE_DEVICES=1 python experiments/exp_63m_full_eval.py
"""
import sys, os
...
CKPT = 'checkpoints/real_synth_combined.pth'
...
# ============================================================
# 1. Forecasting (head fine-tune, grid search best config)
# ============================================================
def eval_forecasting():
    print(f'\n{"="*60}')
    print('1. Forecasting (63M MoE, head fine-tune)')
    print('   Config: lr=0.0001, wd=0.001, bs=64')
    print(f'{"="*60}')
    ...
    for dname, (data, root, fpath, enc_in) in datasets.items():
        for pl in [96, 192, 336, 720]:
            ...
            # Freeze all, unfreeze forecast_head
...
def eval_imputation():
    print(f'\n{"="*60}')
    print('2. Imputation (63M MoE, recon head fine-tune)')
    print(f'{"="*60}')
    ...
    for dname, (data, root, fpath, enc_in) in datasets.items():
        ...
        # Fine-tune recon head (freeze everything else)
        for p in model.parameters(): p.requires_grad = False
        trainable = []
        for name, p in model.named_parameters():
        ...
        if len(trainable) == 0:
            # fallback: tune forecast_head for reconstruction
            for name, p in model.named_parameters():
                if 'forecast_head' in name:
                    p.requires_grad = True; trainable.append(p)
        ...
        for epoch in range(30):
            ...
            for bx, by, _, _ in train_dl:
                ...
                loss_mat = F.mse_loss(out, bx, reduction='none')
                inv = 1.0 - mask
                loss = (loss_mat * inv).sum() / inv.sum().clamp(min=1)
                loss.backward(); optimizer.step(); losses.append(loss.item())
            tl = np.mean(losses)
            if tl < best_loss:
...
# ============================================================
# 3. Classification (linear probe)
# ============================================================
def eval_classification():
    print(f'\n{"="*60}')
    print('3. Classification (63M MoE, linear probe)')
    print(f'{"="*60}')
...
if __name__ == '__main__':
    print('='*60)
    print('63M MoE Full Eval (head fine-tune for all tasks)')
    print(f'Checkpoint: {CKPT}')
    print('='*60)

    fc_results = eval_forecasting()
    eval_imputation()
    eval_classification()
