"""
FiLM Trunk: Fixed basis + input-dependent modulation (scale/shift)
Fixed Trunk과 Hyper Trunk의 중간 — contribution point.

사용법:
  source /opt/miniforge3/etc/profile.d/conda.sh && conda activate timefound
  CUDA_VISIBLE_DEVICES=X python experiments/exp_film_trunk_test.py 2>&1 | tee log/film_trunk_test.log
"""
import sys, os
...
class FiLMTrunkExpert(nn.Module):
    """DeepONet expert with FiLM-modulated trunk.

    Fixed trunk provides stable basis functions.
    Head generates scale & shift to modulate per input → adaptive but stable.

    output = B · (Φ_fixed * scale + shift) + bias
    Head outputs: B [64] + scale [64] + shift [64] = 192 total (was 4288!)
    """
    def __init__(self, branch_dim, width, branch_depth, trunk_depth,
        ...
        # Fixed Trunk MLP (learned but same for all inputs)
        trunk_layers = []
        ...
        # FiLM Head: B (coefficients) + scale + shift
        # 64 + 64 + 64 = 192 (vs Hyper's 4288)
        self.forecast_head = nn.Linear(branch_hidden, width * 3)  # B + scale + shift
        self.recon_head = nn.Linear(branch_hidden, width * 3)
    ...
    def forward(self, branch_input, target_pred_len, head='forecast'):
        ...
        if head == 'forecast':
            head_out = self.forecast_head(z)  # [B, 192]
            bias = self.bias
        else:
            head_out = self.recon_head(z)
            bias = self.recon_bias

        # Parse: B + scale + shift
        B = head_out[:, :self.width]                          # [B, 64]
        scale = head_out[:, self.width:self.width*2]          # [B, 64]
        shift = head_out[:, self.width*2:self.width*3]        # [B, 64]

        # Scale should be centered around 1 (multiplicative modulation)
        scale = 1.0 + 0.1 * scale  # small perturbation around 1
        ...
        Phi = self.trunk_net(t_features)  # [pred_len, 64] — FIXED basis

        # FiLM modulation: Φ_modulated = Φ * scale + shift (per input)
        # Phi: [pred_len, 64], scale/shift: [B, 64]
        Phi_mod = Phi.unsqueeze(0) * scale.unsqueeze(1) + shift.unsqueeze(1)
        # [B, pred_len, 64]

        # DeepONet: B · Φ_modulated
        output = torch.einsum('bp,bqp->bq', B, Phi_mod)  # [B, pred_len]
        return output + bias


class FiLMTrunkModel(nn.Module):
    """Full model with FiLM Trunk experts + MoE routing."""
    ...
    def _run_channel(self, x_ch, target_len, head='forecast'):
        B = x_ch.shape[0]
        router_logits = self.router(x_ch)
        expert_weights = F.softmax(router_logits, dim=-1)

        out = torch.zeros(B, target_len, dtype=x_ch.dtype, device=x_ch.device)
        for i, expert in enumerate(self.experts):
            expert_out = expert(x_ch, target_len, head=head)
    ...
    def forward(self, x_enc, x_mark_enc=None, x_dec=None, x_mark_dec=None,
        ...
        outputs = []
        for ch in range(C):
            outputs.append(self._run_channel(x_enc[:, :, ch], target_pred_len, 'forecast'))
    ...
    def reconstruct(self, x_enc):
        ...
        outputs = []
        for ch in range(C):
            outputs.append(self._run_channel(x_enc[:, :, ch], S, 'recon'))
        output = torch.stack(outputs, dim=-1)
...
def run_experiment():
    ...
    # ============ Forecasting ============
    print('=' * 60)
    print('FORECASTING (FiLM Trunk, from-scratch)')
    print('=' * 60)
    ...
    for dname, (data, root, fpath, enc_in) in datasets.items():
        for pl in [96, 336]:
            ...
            model = FiLMTrunkModel(args).to(device)
    ...
    # ============ Classification ============
    print('\n' + '=' * 60)
    print('CLASSIFICATION (FiLM Trunk, from-scratch)')
    print('=' * 60)
    ...
    # ============ Summary ============
    print('\n' + '=' * 60)
    print('SUMMARY: FiLM Trunk (from-scratch, real data)')
    print('=' * 60)
